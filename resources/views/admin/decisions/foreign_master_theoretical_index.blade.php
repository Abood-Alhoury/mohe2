...
@section('title', 'إصدار قرارات تعادل الماجستير الخارجي النظري')

@section('content')

<!-- NAV TABS FOR DECISION TYPES -->
@include('admin.decisions._nav_tabs', ['active' => 'foreign_master_theoretical'])

<div class="row g-3" dir="rtl">

    {{-- ============================================================
         العمود الأيمن: نموذج إصدار قرارات تعادل الماجستير الخارجي النظري
    ============================================================ --}}
        ...
        <div class="card border-0 shadow-sm overflow-hidden h-100" style="border-top: 3.5px solid var(--heritage-gold) !important; border-radius: 4px;">
            ...
            <div class="card-header py-2.5 px-3 text-white" style="background-color: var(--imperial-navy) !important;">
                <h5 class="mb-0 fs-6 fw-bold text-white d-flex align-items-center gap-2 text-start" dir="rtl">
                    <i class="fa-solid fa-book-open-reader" style="color: var(--heritage-gold-light);"></i>
                    <span>إصدار ورصد قرارات الماجستير النظري والأهلية</span>
                </h5>
            </div>
            <div class="card-body p-3.5 bg-white" dir="rtl">
                <!-- Info banner: Has eligibility -->
                <div class="alert alert-info py-2 px-2.5 mb-3 border-0 small d-flex align-items-start gap-2" style="background-color: #f0fdf4; color: #166534; font-size: 0.8rem; border-radius: 4px;">
                    <i class="fa-solid fa-certificate fs-6 mt-0.5 text-success flex-shrink-0"></i>
                    <div>
                        <strong>مسار نظري (تدريس نظري):</strong> يتطلب إثبات خبرة سنتين واجتياز المقابلة العلمية، <u>ويصدر له قرار أهلية تدريس إضافةً لقرار التعادل</u>.
                    </div>
                </div>

                <form action="{{ route('admin.foreign_master_theoretical_decisions.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf

                    {{-- اختر طلب الماجستير الخارجي النظري --}}
                    <div class="mb-3 text-start" x-data="{
                        ...
                        items: [
                            @foreach($approvedApps as $ap)
                            {
                                ...
                                type: '{{ addslashes($ap->request_type ?? 'ماجستير خارجي نظري') }}'
                            },
                            @endforeach
                        ],
                        get filteredItems() {
                        ...
                        <label class="form-label fw-bold small d-block text-start mb-1" style="color: var(--imperial-navy);">
                            اختر طلب الماجستير الخارجي النظري الموافق عليه :
                        </label>
                            ...
                            <div x-show="open" 
                                ...
                                <template x-for="item in filteredItems" :key="item.id">
                                    <div @click="select(item)" 
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <span class="badge bg-primary-subtle text-primary fs-9" x-text="item.appNo"></span>
                                        </div>
                                ...
                                <template x-if="filteredItems.length === 0">
                                    <div class="p-3 text-center text-muted small">
                                        <i class="fa-solid fa-inbox me-1"></i> لا توجد طلبات ماجستير خارجي نظري مطابقة بحالة بانتظار إصدار القرار.
                                    </div>
                                </template>
                            </div>
                        </div>
                            ...
                            <div class="mt-2 text-start">
                                <a :href="'/admin/reports/' + selectedId + '/generate-decision'" 
                                   ...
                                   class="btn btn-xs btn-outline-primary d-inline-flex align-items-center gap-1">
                                    ...
                                    <span>توليد ومعاينة نص القرار النظري وقرار الأهلية والـ PDF</span>
                                </a>
                            </div>
                        </template>
                    </div>
                    ...
                    {{-- ملف قرار التعادل النظري --}}
                    <div class="mb-2.5 text-start">
                        <label class="form-label fw-bold small d-block text-start mb-1" style="color: var(--imperial-navy);">
                            ملف قرار تعادل الماجستير النظري (PDF) <span class="text-danger">*</span> :
                        </label>
                    ...
                    <div class="p-2.5 bg-light rounded border mb-2.5 text-start">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            ...
                            <span class="badge bg-success-subtle text-success fs-9 border border-success">قرار رسمي</span>
                        </div>
                    ...
                    <div class="mb-3 text-start">
                        ...
                        <textarea name="notes" class="form-control form-control-sm text-start" rows="2" placeholder="أدخل أي ملاحظات خاصة بقرار الماجستير الخارجي النظري..."></textarea>
                    </div>
                    ...
                    <button type="submit" class="btn btn-solid-navy btn-sm w-100 fw-bold py-2 shadow-2xs d-flex align-items-center justify-content-center gap-1.5">
                        ...
                        <span>حفظ وإصدار القرار النظري والأهلية وإشعار الجامعة</span>
                    </button>
                </form>
            </div>
        </div>
    ...
    {{-- ============================================================
         العمود الأيسر: جدول سجل قرارات تعادل الماجستير الخارجي النظري الصادرة
    ============================================================ --}}
        ...
        <div class="card border-0 shadow-sm overflow-hidden" style="border-top: 3.5px solid var(--imperial-navy) !important; border-radius: 4px;">
            ...
            <div class="card-header py-2 px-3 bg-light border-bottom d-flex align-items-center justify-content-between flex-wrap gap-2">
                <h5 class="mb-0 fs-6 fw-bold d-flex align-items-center gap-2" style="color: var(--imperial-navy);">
                    ...
                    <span>سجل قرارات تعادل الماجستير الخارجي النظري والأهلية الصادرة</span>
                </h5>

                <form method="GET" action="{{ route('admin.foreign_master_theoretical_decisions.index') }}" class="d-flex align-items-center gap-1.5 m-0">
                    ...
                    @if(!empty($search))
                        <a href="{{ route('admin.foreign_master_theoretical_decisions.index') }}" class="btn btn-sm btn-outline-danger px-2" title="إلغاء البحث">
                    ...
                    <table class="table table-hover align-middle mb-0 text-center" style="font-size: 0.84rem;">
                        <thead style="background-color: #f8fafc; color: var(--imperial-navy); border-bottom: 2px solid #e2e8f0;">
                            <tr>
                                ...
                                <th class="py-2.5 px-2">قرار التعادل النظري</th>
                                <th class="py-2.5 px-2">قرار الأهلية</th>
                        ...
                        <tbody>
                            @forelse($issuedDecisions as $dec)
                                ...
                                <tr>
                                    ...
                                    <!-- Theoretical Equivalence Decision Number -->
                                    <td class="py-2.5 px-2">
                                    ...
                                    <!-- Eligibility Decision Number & Badge -->
                                    <td class="py-2.5 px-2">
                                        @if($dec->eligibility_decision_no)
                                            <span class="badge bg-success-subtle text-success border border-success px-2 py-1 fw-bold d-inline-flex align-items-center gap-1">
                                                <i class="fa-solid fa-award"></i>
                                                <span>{{ $dec->eligibility_decision_no }}</span>
                                            </span>
                                            @if($dec->eligibility_decision_date)
                                                <div class="text-muted fs-9 mt-0.5">{{ format_sys_date($dec->eligibility_decision_date) }}</div>
                                            @endif
                                        @else
                                            <span class="badge bg-light text-muted border fs-9">غير مسجل</span>
                                        @endif
                                    </td>
                                        ...
                                        <div class="d-inline-flex align-items-center gap-1.5">
                                            @if($dec->file_path)
                                                <a href="{{ Storage::url($dec->file_path) }}" 
                                                   ...
                                                   title="تحميل قرار التعادل / الحزمة المدمجة">
                                                    <i class="fa-solid fa-file-pdf text-danger"></i>
                                                    <span>القرار</span>
                                                </a>
                                            @endif

                                            @if($dec->eligibility_file_path && $dec->file_path !== $dec->eligibility_file_path)
                                                <a href="{{ Storage::url($dec->eligibility_file_path) }}" 
                                                   target="_blank" 
                                                   class="btn btn-xs btn-outline-success px-2 py-1 shadow-2xs d-inline-flex align-items-center gap-1"
                                                   title="تحميل وثيقة الأهلية منفصلة">
                                                    <i class="fa-solid fa-file-certificate text-success"></i>
                                                    <span>الأهلية</span>
                                                </a>
                                            @endif
                                            ...
                                            @if($app)
                                                <a href="{{ route('admin.reports.generate_decision', $app->id) }}" 
                                                   ...
                                                   title="معاينة نموذج القرار الرسمي والأهلية وطباعته">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-4 text-muted text-center">
                                        ...
                                        لا توجد قرارات تعادل ماجستير خارجي نظري مسجلة حالياً.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
